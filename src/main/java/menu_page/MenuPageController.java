    ...
    private List<Category> getCategoryData() {
        ...
        try (
            ...
            while (queryResult.next()) {
                ...
                ResultSet totalQuantity = statement2.executeQuery(countItemsPerCategory + queryResult.getInt("category_id") + "\";");
    ...
    private List<MenuItem> getMenuItemData(Category category) {
        ...
        try (
            ...
            while (queryCategoryId.next()) {
                String fetchMenuItems = "SELECT * FROM menu_item WHERE category_id=" + queryCategoryId.getInt("category_id");
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
            ...
            categoryListener = category -> {
                try {
                    ...
                    for (MenuItem item : categoryItems) {
                        FXMLLoader fxmlLoader = new FXMLLoader(MainApplication.class.getResource("/trailfood/menu_page/MenuItem.fxml"));
                        VBox vBox = fxmlLoader.load();

                        MenuItemController menuItemController = fxmlLoader.getController();
                        menuItemController.setData(item);
        ...
        try {
            for (Category category : categories) {
                FXMLLoader fxmlLoader = new FXMLLoader(MainApplication.class.getResource("/trailfood/menu_page/Category.fxml"));
                VBox vBox = fxmlLoader.load();

                CategoryController categoryController = fxmlLoader.getController();
                categoryController.setData(category, categoryListener);
